	...
	try {
		...
		if (!nameRegex.test(name)) {
			return res
				.status(400)
				.send(
					"Il nome deve essere lungo tra 2 e 50 caratteri e può contenere solo lettere, spazi, trattini e caratteri accentati"
				);
		}
		...
		if (!surnameRegex.test(surname)) {
			return res
				.status(400)
				.send(
					"Il cognome deve essere lungo tra 2 e 50 caratteri e può contenere solo lettere, spazi, trattini e caratteri accentati"
				);
		}
		...
		if (!usernameRegex.test(username)) {
			return res
				.status(400)
				.send(
					"Lo username deve essere lungo tra 3 e 30 caratteri e può contenere solo lettere, numeri e underscore"
				);
		}
		...
		if (!passwordRegex.test(password)) {
			return res
				.status(400)
				.send(
					"La password deve essere lunga almeno 8 caratteri e contenere almeno una maiuscola, un carattere speciale e un numero"
				);
		}
      ...
      `,
		...
		res
			.status(200)
			.json({
				message: "Controlla la tua email per verificare il tuo indirizzo.",
			});
	} catch (error) {
...
router.get("/verify-email", verifyEmailLimiter, async (req, res) => {
	...
	try {
		...
		if (!user) {
			return res
				.status(400)
				.json({ message: "Token non valido o utente non trovato" });
		}
		...
		if (type === "registration") {
			return res.redirect(
				`${process.env.FRONTEND_URL}/account/other/registrationSuccess`
			);
		} else if (type === "reset-password") {
			return res.redirect(
				`${process.env.FRONTEND_URL}/account/password/recoverPassword`
			);
		} else {
...
router.post("/resend-verification", async (req, res) => {
	...
	try {
		...
		if (!user) {
			return res
				.status(404)
				.json({ message: "Utente non trovato o già verificato" });
		}
    ...
    `,
		...
		res
			.status(200)
			.json({ message: "Email di verifica reinviata con successo" });
	} catch (error) {
		res
			.status(500)
			.json({ message: "Errore durante il reinvio dell'email di verifica" });
	}
});
	...
	try {
		...
		if (user && (await bcrypt.compare(password, user.password))) {
			const token = jwt.sign(
				{ userId: user._id, username: user.username, priority: user.priority },
...
router.get("/verify-token", authMiddleware, (req, res) => {
	res.json({ valid: true, priority: req.user.priority });
});
